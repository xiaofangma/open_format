...
function parseInline(text: string): React.ReactNode {
  ...
  while ((m = regex.exec(text)) !== null) {
    ...
    const full = m[0]
    if (full.startsWith('**')) {
      parts.push(<strong key={m.index} style={{ fontWeight: 700, color: '#000000' }}>{m[2]}</strong>)
    } else if (full.startsWith('==')) {
      parts.push(<mark key={m.index} style={{ background: '#f0e2c8', padding: '0 3px', borderRadius: '3px', color: '#000000' }}>{m[4]}</mark>)
    } else if (full.startsWith('~~')) {
      parts.push(<del key={m.index} style={{ textDecoration: 'line-through', opacity: 0.6 }}>{m[5]}</del>)
    } else if (full.startsWith('`')) {
      parts.push(<code key={m.index} style={{ background: '#ececec', padding: '1px 4px', borderRadius: '3px', fontSize: '0.88em', fontFamily: 'ui-monospace, monospace', color: '#000000' }}>{m[6]}</code>)
    } else if (full.startsWith('[')) {
